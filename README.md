...
- YYYY-MM-DD: Version 1.1 published, with the following changes:
    ...
    - Canonical expression for Streett acceptance now has all pairs reversed, and generalized Streett is defined ([#62](https://github.com/adl/hoaf/issues/62))
...
Generic Streett acceptance is simply the dual of generalized Rabin acceptance, it generalize Streett by allowing multiple `Fin(.)` terms in disjunctive clauses (called generalized pairs):
    ...
    acc-name: generalized-Streett 2 3 2
    Acceptance: 7 (Inf(0)|Fin(1)|Fin(2)|Fin(3))&(Inf(4)|Fin(5)|Fin(6))

The first parameter of `generalized-Streett` gives the number of generalized pairs and the following parameters give the number of `Fin(.)` terms in the corresponding pairs.
...
This however does not fit our strict definition of `acc-name: Streett 2` (which would be `Acceptance 4: (Inf(0)|Fin(1))&(Inf(2)|Fin(3))`, i.e., with 4 sets in exactly this order).
